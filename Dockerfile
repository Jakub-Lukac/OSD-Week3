...
COPY --from=build /app/dist/osd-week3-signals/browser /usr/share/nginx/html

# docker build -t my-docker-app:1.0.0 .
# . dot at end tells docker to use the current directory as context for the build, which includes the Dockerfile and all necessary files for the build process.